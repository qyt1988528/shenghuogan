...
//        self::IMAGE_DOWNLOAD_TIMEOUT,
        ...
        return function( $response){
            if($response->getStatusCode() != 200){
                ...
                if(isset($result['error_message']) && in_array($result['error_message'],$this->errorMessage)){
                    ...
                    throw new \Exception("Network Error. Please Try Again Later!",1003);
                }
            }
            /*
     ...
     */
    public function faceDetect($image,$source=''){
...
//        $updateData['updated_at'] = date('Y-m-d H:i:s');
    ...
    public function mergeFacePro($params){
        if(empty($params['image_url']) && empty($params['image_base64'])){
            $result['base64str'] = '';
            $result['code'] = 101;
            $result['msg'] = $this->translate->_('Please upload image !');
            return $result;
        }
        //通过sku找模板图
        ...
        if(!empty($params['image_base64'])){
            ...
            $params['merge_url'] = $params['image_url'];
        }
                ...
                foreach ($data['faces'] as $df){
                    if(isset($df['attributes']['headpose'])){
                        ...
                        if(abs($pitchAngle) >= self::NWDN_FACE_TASK_LIMIT_ANGLE){
                            ...
                            if($pitchAngle<0){
                                $ret['pitch_angle'] = 3;
                                $ret['pitch_angle_message'] = '';//抬头
                            }else{
                                $ret['pitch_angle'] = 2;
                                $ret['pitch_angle_message'] = '';//低头
                            }
                        }else{
                            $ret['pitch_angle'] = 1;
                        }
                        ...
                        if(abs($rollAngle) >= self::NWDN_FACE_TASK_LIMIT_ANGLE){
                            ...
                            if($rollAngle<0){
                                $ret['roll_angle'] = 3;
                                $ret['roll_angle_message'] = '';//右旋转
                            }else{
                                $ret['roll_angle'] = 2;
                                $ret['roll_angle_message'] = '';//左旋转
                            }
                        }else{
                            $ret['roll_angle'] = 1;
                        }
                        ...
                        if(abs($yawAngle) >= self::NWDN_FACE_TASK_LIMIT_ANGLE){
                            ...
                            if($yawAngle<0){
                                $ret['yaw_angle'] = 3;
                                $ret['yaw_angle_message'] = '';//右摇头
                            }else{
                                $ret['yaw_angle'] = 2;
                                $ret['yaw_angle_message'] = '';//左摇头
                            }
                        }else{
                            $ret['yaw_angle'] = 1;
                        }
