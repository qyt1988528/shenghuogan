...
class Helper extends Api
{
     ...
     * @return array
     */
    protected function _getHeader(string $path ,array $params)
    {
...
//            'background_url' => $background,
        ...
        $header  = $this->_getHeader('rs2c/createFaceTask',$params);
        $response = $this->_client->request('POST', '/rs2c/createFaceTask',[
            "body" => base64_encode(json_encode($params)),
            "headers" => $header,
        ]);
...
//        $data['background_url'] = $background;
    ...
    public function getFaceTask($taskId,$source=''){
        ...
        $header  = $this->_getHeader('rs2c/getFaceTask',$params);
        $response = $this->_client->request('POST', '/rs2c/getFaceTask',[
            "body" => base64_encode(json_encode($params)),
            "headers" => $header,
        ]);
