...
use MDK\Application;
use MDK\Config;
use MDK\Behaviour\DI as BehaviourDI;
use MDK\Package\Exception\InvalidManifest;
use MDK\Package\Model\Package;
use Phalcon\DI;
...
class Manager
{
     ...
     *
     * @param Package|string $package Package.
     *
     * @return string
     */
    ...
    public function removePackage($package)
    {
        ...
        $metadataFile = Config::CONFIG_METADATA_PACKAGES . '/' .
     ...
     * @return void
     */
    public function exportPackage(Package $package, array $params = [])
    {
    ...
    {
        $directory = $this->dir->root() . str_replace('_', DS, 'var_packages_');
        if ($checkDir) {
     ...
     *
     * @param Package[] $packages      Packages array.
    ...
    public function generateMetadata($packages = null, $checkManifest = false)
    {
        ...
        $packagesMetadataDirectory = Config::CONFIG_METADATA_PACKAGES;

        Utilities::fsCheckLocation($packagesMetadataDirectory);

        $config = ['installed' => CORE_VERSION, 'events' => [], 'modules' => []];
        foreach ($packages as $package) {
            if (!$package->enabled) {
                continue;
            }
            ...
            // Get package events.
            if (
                (in_array($package->type, [self::PACKAGE_TYPE_MODULE])) &&
                !$package->is_system
            ) {
            ...
            if (!empty($data) && !empty($data['module'])) {
                continue;
            }
     ...
     * @return string
     */
    private function _getPackageFullName(Package $package, $appendVersion = false, $separator = '-')
    {
